...
function Food() {
    ...
    const smallDivs = Array.from({ length: 47 });
    ...
    async function foodSet() {
        // console.log(await fetch("https://www.themealdb.com/api/json/v1/1/categories.php"))
        let res = await fetch("https://www.themealdb.com/api/json/v1/1/categories.php");
        let res1 = res.json();
        const res2 = await res1.then();
        setRecipe([...recipe,res2.categories]);
        // console.log(res2.categories);

        let response = await fetch("https://www.themealdb.com/api/json/v1/1/filter.php?a=indian");
        let response1 = response.json();
        let response2 = await response1.then();
        // console.log(response2.meals);
        setTheFood(response2.meals);
    }
    foodSet();

    async function setTheFood(mealsId) {
        for (let i = 0; i < mealsId.length; i++) {
            if (count == 0) {
                console.log(mealsId[i].idMeal);
                let result = await fetch("https://www.themealdb.com/api/json/v1/1/lookup.php?i=" + mealsId[i].idMeal);
                let result1 = result.json();
                let result2 = await result1.then();
                // setFood(result2.meals[0].strMealThumb);
                // setFoodName(result2.meals[0].strMeal);
                console.log(result2.meals[0].strMeal);
        ...
        <div className="recipeOuter">
            <div className="recipes">
                ...
                <div className="insertFood">
                    {/* {setFoodName(mealsFood[1].strMeal)}
                    {setFood(mealsFood[1].strMealThumb)} */}
                    {smallDivs.map((_, index) => (
                        <div key={index} className="smallDiv" onClick={(e) => {
                            // navigate("/singleFood", {state: {name: e.target.strMeal}});
                            for(let i=0; i<mealsFood.length; i++){
                                if(e.target.id == i){
